...
class TiM5xx_Var : public SensorVariable {
 public:
  TiM5xx_Var(std::string name, std::string comName, uint16_t idx,
             VariableDirection rwDir, AccessLevel readAccessLvl,
             AccessLevel writeAccessLvl, int32_t eventIdx)
      : SensorVariable(name, comName, idx, rwDir, readAccessLvl, writeAccessLvl,
                       eventIdx){};
  ~TiM5xx_Var(){};
...
class EtherHostCoLaDialect_TiM5xx_Var : public TiM5xx_Var<Enum8_t> {
 public:
  ...
  static SensorVariable* Create() {
    return new EtherHostCoLaDialect_TiM5xx_Var;
  }
  SensorResult GetBasic(int8_t& value);
  SensorResult GetBasic(int16_t& value);
  SensorResult GetBasic(int32_t& value);
